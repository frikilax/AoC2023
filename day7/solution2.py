...
def parse_hand(hand):
    ...
    for card in hand:
        if card == 'J':
            for c in card_mapping.keys():
                parsed[c] = parsed.get(c, 0) + 1
        else:
...
def score_hand(hand):
    ...
    if 5 in counts:
        ...
        score = 6
    elif 3 in counts and 'J' in hand and len([x for x in counts if x == 3]) == 2:
        score = 5
    elif 3 in counts and 2 in counts and not 'J' in hand:
        score = 5
    elif 3 in counts:
        score = 4
